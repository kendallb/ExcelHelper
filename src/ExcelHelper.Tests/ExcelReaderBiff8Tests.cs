...
{
    [TestClass]
    public class ExcelReaderBiff8Tests
    {
        [TestMethod]
        public void ReadCellTest()
        {
            using (var stream = new MemoryStream()) {
                ...
                using (var book = new C1XLBook()) {
                    ...
                    sheet[0, 6].Value = "yes";
                    sheet[0, 7].Value = c;
                    sheet[0, 8].Value = null;
                    sheet[0, 9].Value = guid.ToString();
                    sheet[0, 10].Value = ts.ToString();
                ...
                stream.Position = 0;
                using (var excel = new ExcelReaderBiff8(stream)) {
                    // Check the column and row counts are correct
                    Assert.AreEqual(11, excel.TotalColumns);
                    ...
                    Assert.AreEqual("yes", excel.GetCell<string>(0, 6));

                    // Test character
                    Assert.AreEqual('c', excel.GetCell<char>(0, 7));
                    Assert.AreEqual("c", excel.GetCell<string>(0, 7));

                    // Test null
                    Assert.AreEqual("", excel.GetCell<string>(0, 8));
                    Assert.AreEqual(null, excel.GetCell<int?>(0, 8));

                    // Test guid
                    Assert.AreEqual(guid, excel.GetCell<Guid>(0, 9));
                    Assert.AreEqual(guid.ToString(), excel.GetCell<string>(0, 9));

                    // Test TimeSpan
                    Assert.AreEqual(ts, excel.GetCell<TimeSpan>(0, 10));
                    Assert.AreEqual(ts.ToString(), excel.GetCell<string>(0, 10));
        ...
        public void ReadBiff8Test()
        {
            using (var stream = new MemoryStream()) {
                ...
                using (var excel = new ExcelReaderBiff8(stream)) {
                    // Test all number conversions
                    Assert.AreEqual("one", excel.GetCell<string>(0, 0));
                }
            }
        }
        ...
        public void IgnoreExceptionsTest()
        {
            using (var stream = new MemoryStream()) {
                ...
                using (var excel = new ExcelReaderBiff8(stream)) {
                    ...
                    var message =
                        @"Type: 'ExcelHelper.Tests.ExcelReaderBiff8Tests+TestRecord'" + "\r\n" +
                        @"Row: '2' (1 based)" + "\r\n" +
                        @"Column: '3' (1 based)" + "\r\n" +
